...
module.exports = {
    ...
    run: async(client, message, args) => {
        const size = args.toString().split(/\s+/);
        let msg = await message.channel.send(size[0]);
        if(args.toString().split(" ").length !== 1) {
            let msg = await message.channel.send("Too many arguments. Must only provide 1 message id");
            await msg.delete({ timeout: 35000 }).catch(err => console.log(err));
        }
        else {
            try {
                ...
                if(fetchedMessage) {
                    ...
                    collector.on('collect', msg => {
                        ...
                        if(msg.content.toLowerCase() === 'c!done') {
